    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border bg-card">
        <table className="w-full text-sm">
          <thead className="border-b text-left text-muted-foreground"><tr><th className="p-3">Name</th><th className="p-3">Email</th><th className="p-3">Role</th><th className="p-3">Status</th><th className="p-3">Blogs</th><th className="p-3">Joined</th><th className="p-3"></th></tr></thead>
          <tbody>{users.map(u => (
            <tr key={u.id} className="border-b last:border-0">
              ...
              <td className="p-3">
                <Select defaultValue={u.role} onValueChange={(v) => start(async () => { await updateUserRoleAction(u.id, v as Role); router.refresh(); })}>
                  ...
                  <SelectContent>{(["ADMIN","EDITOR","AUTHOR","GUEST"] as Role[]).map(r => <SelectItem key={r} value={r}>{r}</SelectItem>)}</SelectContent>
                </Select>
              </td>
              <td className="p-3">
                <Select defaultValue={u.status || "active"} onValueChange={(v) => start(async () => { await (await import("@/server/actions/users")).updateUserStatusAction(u.id, v); router.refresh(); })}>
